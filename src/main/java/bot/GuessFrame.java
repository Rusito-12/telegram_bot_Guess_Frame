...
public class GuessFrame implements LongPollingSingleThreadUpdateConsumer {
    ...
    public GuessFrame(TelegramClient telegramClient, GameManager gameManager, List<Movie> movies) {
    ...
    public void consume(Update update) {
        ...
        String userMassageText = message.getText();

        if (userMassageText.equalsIgnoreCase("/start")){
            startNewGame(chatId);
        }else {
            handleUserGuess(chatId, userMassageText);
        }
    }

    private void handleUserGuess(String chatId, String userMassageText) {
        ...
        if (lastQuestion.isRightAnswer(userMassageText)) {
            ...
            sendMessage(chatId, "К сожалению ответ неверный! Правильны ответ: %s"
    ...
    private void sendPhoto(String chatId, String imagePath, String caption, ReplyKeyboard keyboard) {
        try(InputStream inputStream = getClass().getResourceAsStream(imagePath)){
            ...
            if (inputStream == null) {
                System.out.println("Изображение % не найдено".formatted(imagePath));
                return;
            }
